...
export default class QueryBuilder {
    ...
    getFragment(type) {
        ...
        if (getObjectFragment)
            return getObjectFragment(type);
    ...
    getListableScalarFields(type, filterFields = this.filterListFields.bind(this), max = 4) {
        ...
        const simpleNodeFields = Object.values(type.getFields())
            .map((field) => ({ field, fType: field.type }))
    ...
    serializeInputValue(input, argType) {
        ...
        if (isPlainObject(input)) {
            ...
            const objectItems = Object.entries(input)
                ...
                .map(([k, v]) => `${k}: ${this.serializeInputValue(v, objectFields[k].type)}`)
                .join(', ');
            return `{${objectItems}}`;
        }
    ...
    serializeArgsInline(args, argDefinitions) {
        ...
        const serializedArgs = Object.entries(args)
            .filter(([, v]) => v !== undefined)
            .map(([k, v]) => `${k}: ${this.serializeInputValue(v, argsByName[k].type)}`)
            .join(', ');
